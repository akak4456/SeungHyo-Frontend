...
function drawYearTitle(ctx, yearText) {
	...
	ctx.fillText(
		yearText,
		-(actualWidth / 2) - RECT_SIZE,
		actualHeight + paddingHorizontal
	);
	ctx.restore();
...
function drawGrass(ctx, daysTextEnd, colorData) {
	const maxLevel = Math.max(
		...colorData.map((subarray) =>
			Math.max(...subarray.map((item) => item.colorLevel))
		)
	);
	...
	colorData.forEach((colorDatum, columnIndex) => {
		...
		if (colorDatum[colorDatum.length - 1].colorLevel >= 0) {
			monthDivideData[colorDatum[colorDatum.length - 1].date.getMonth()]
				.bottomWidth++;
		}
		for (let rowIndex = 0; rowIndex < colorDatum.length; rowIndex++) {
			if (
				rowIndex + 1 < colorDatum.length &&
				colorDatum[rowIndex].date.getMonth() !=
					colorDatum[rowIndex + 1].date.getMonth()
			) {
				monthDivideData[colorDatum[rowIndex].date.getMonth()].midY =
					rowIndex + 1;
			}
	...
	ctx.lineTo(
		topStartX,
		grassStartY + startBoxIdx * (RECT_SIZE - RECT_BORDER_SIZE)
	);
	...
	monthDivideData.forEach((divideData, columnIndex) => {
		...
		ctx.lineTo(
			...
			grassStartY + 7 * (RECT_SIZE - RECT_BORDER_SIZE)
		);
		ctx.strokeStyle = divideBorderColor;
		bottomStartX += divideData.bottomWidth * (RECT_SIZE - RECT_BORDER_SIZE);
		ctx.stroke();
		...
		divideBorderColor = GRASS_MONTH_DIVIDER_BORDER_UNSELECTED_COLOR;
		if (
			divideData.isSelected ||
			(columnIndex + 1 < monthDivideData.length &&
				monthDivideData[columnIndex + 1].isSelected)
		) {
			divideBorderColor = GRASS_MONTH_DIVIDER_BORDER_SELECTED_COLOR;
		}
		...
		ctx.lineTo(
			topStartX,
			grassStartY + divideData.midY * (RECT_SIZE - RECT_BORDER_SIZE)
		);
		ctx.strokeStyle = divideBorderColor;
		ctx.stroke();
	...
	monthsText.forEach((monthText, index) => {
		...
		ctx.fillText(
			...
			actualHeight + (grassStartY - actualHeight) / 2
		);
		startX += wholeWidth;
	});
	ctx.restore();
...
const GrassGraph = (props) => {
	...
	useEffect(() => {
		...
		if (canvas) {
			...
			if (width == 0) {
				setDimensions({
					width: grassEnd + paddingHorizontal,
					height: RECT_SIZE * 8 + RECT_BORDER_SIZE,
				});
			}
		}
	}, [dimensions, props]);
